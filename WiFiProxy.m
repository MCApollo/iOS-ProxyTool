...
- (void)setProxy:(NSString *)ipaddr port:(NSUInteger)port mode:(int)mode {
  SCPreferencesRef prefRef =
      SCPreferencesCreate(NULL, CFSTR("set_proxy"), NULL);

  SCPreferencesLock(prefRef, true);

  CFStringRef currentSetPath =
      SCPreferencesGetValue(prefRef, kSCPrefCurrentSet);

  NSDictionary *currentSet = (__bridge NSDictionary *)SCPreferencesPathGetValue(
      prefRef, currentSetPath);
  if (currentSet) {
    NSDictionary *currentSetServices =
        currentSet[cfs2nss(kSCCompNetwork)][cfs2nss(kSCCompService)];

    NSDictionary *services = (__bridge NSDictionary *)SCPreferencesGetValue(
        prefRef, kSCPrefNetworkServices);

    NSData *data = [NSPropertyListSerialization
        dataWithPropertyList:services
                      format:NSPropertyListBinaryFormat_v1_0
                     options:0
                       error:nil];
    NSMutableDictionary *nservices = [NSPropertyListSerialization
    ...
    for (NSString *key in currentSetServices) {
      ...
      if (service && [@"Wi-Fi" isEqualToString:name]) {
        ...
        NSMutableDictionary *proxies =
            nservices[wifiServiceKey][(__bridge NSString *)kSCEntNetProxies];

        [self setProxyInner:ipaddr port:port proxyDict:proxies mode:mode];
      }
    }

    SCPreferencesSetValue(prefRef, kSCPrefNetworkServices,
                          (__bridge CFPropertyListRef)nservices);
...
- (void)setProxyInner:(NSString *)ipaddr
                 port:(NSUInteger)port
            proxyDict:(NSMutableDictionary *)proxies
                 mode:(int)mode {
